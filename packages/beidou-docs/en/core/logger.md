...
Beidou offers a few loggers for different scenarios:
...
For developers who want to know more details about dispatch in Beidou, they can easily use `App Logger` to make that happen:
...
Generally, Beidou will only write logs in levels higher than `INFO`, so it means that `NONE` and `DEBUG` information will be lost in files.
...
By default, Beidou will only print out `INFO`, `WARN` and `ERROR` in terminal. `logger.consoleLevel`(default: `INFO`) is defined as the logger level in terminal. Similarly, it can be changed as following:
...
Transport can be considered as a tunnel to transfer data in Beidou. A logger contains multiple transports, for example the one by default contains `fileTransport` and `consoleTransport`.
...
One common requirement you can find in enterprise logs is automatic log sharding, which offers a convenient way for management. Luckily, Beidou takes [egg-logrotator](https://github.com/eggjs/egg-logrotator) as built-in solution to meet the need.

### Daily Sharding

This is the default way in Beidou to cut the logs into files named by `.log.YYYY-MM-DD` at every `00:00`. For example, `example-app-web.log` will be cut into files as follow, `example-app-web.log.YYYY-MM-DD`.

### Size Sharding

The log file also can be cut into ones by size. For example, Beidou will process `egg-web.log` when its size reach 2G:
...
Generally, requests are frequent events to Web services, so writing logs into disk after each event will cause more unexpected I/O. Beidou takes following strategy to write logs:

> Logs will be firstly transferred into memory, and then Beidou will asynchronously write them into files by second.
