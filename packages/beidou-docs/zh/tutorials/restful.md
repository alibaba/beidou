...
首先，我们先按照前面的设计来注册[路由](../basic/router.md)，框架提供了一个便捷的方式来创建 RESTful 风格的路由，并将一个资源的接口映射到对应的 controller 文件。在 `app/router.js` 中：
...
在 [controller](../basic/controller.md) 中，我们只需要实现 `app.resources` 约定的 [RESTful 风格的 URL 定义](../basic/router.md#restful-风格的-url-定义) 中我们需要提供的接口即可。例如我们来实现创建一个 topics 的接口：
...
在 [service](../basic/service.md) 中，我们可以更加专注的编写实际生效的业务逻辑。
...
在 `app/middleware` 目录下新建一个 `error_handler.js` 的文件来新建一个 [middleware](../basic/middleware.md)
