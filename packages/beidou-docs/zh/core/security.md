...
- 利用 [extend](https://github.com/eggjs/egg/blob/master/docs/source/zh-cn/basic/extend.md) 机制扩展了 Helper API, 提供了各种模板过滤函数，防止钓鱼或 XSS 攻击。
- 常见 Web 安全头的支持。
- CSRF 的防御方案。
...
match 和 ignore 使用方法和格式与[中间件通用配置](../basic/middleware.md#match和ignore)一致。
