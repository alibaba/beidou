...
- ${app.name}-web.log: 应用 Web 相关日志，用于记录 Web 行为相关的日志。
- common-error.log: 任何 logger.error(err) 输出的日志都会输出到这里，出现异常的时候重点查看此日志。
- egg-web.log: Chair 内核、内置中间件日志。
- sofa-mvc-digest.log: Web 请求响应 digest 日志。
- rpc-client-digest.log: HSF / tr 调用 digest 日志。
- httpclient-digest.log: httpclient / urllib 调用 digest 日志。
- disaster-digest.log: SLA 限流日志。
