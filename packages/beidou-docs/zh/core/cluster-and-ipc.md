...
你可以在应用或插件根目录下的 `agent.js` 中实现你自己的逻辑（和[启动自定义](../basic/app-start.md) 用法类似，只是入口参数是 agent 对象）
...
Worker 进程负责处理真正的用户请求和[定时任务](../basic/schedule.md)的处理。而 Egg 的定时任务也提供了只让一个 Worker 进程运行的能力，**所以能够通过定时任务解决的问题就不要放到 Agent 上执行**。
...
在实际项目中，我们可以采用方案一用于兜底，结合方案三或者方案二的一种用于提升数据更新的实时性。而在这个示例中，我们会通过 IPC + [定时任务](../basic/schedule.md)来同时实现这三种缓存更新方案。
