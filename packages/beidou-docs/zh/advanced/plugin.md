...
2. 插件需要在 `package.json` 中的 `eggPlugin` 节点指定插件特有的信息
    ...
    - `{Array} env` - 只有在指定运行环境才能开启，具体有哪些环境可以参考[运行环境](../deployment/env.md)。此配置是可选的，一般情况下都不需要配置。
...
框架已内置插件列表：详见[内置插件](../basic/plugins.md)


### 根据环境配置

插件还支持 `plugin.{env}.js` 这种模式，会根据[运行环境](../deployment/env.md)加载插件配置。
