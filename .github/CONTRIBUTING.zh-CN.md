...
有任何疑问，欢迎提交 [issue](https://github.com/alibaba/beidou/issues)，
或者直接修改提交 [PR](https://github.com/alibaba/beidou/pulls)!
...
随后 beidou 团队成员会确认 issue 意图，更新合适的标签，关联 milestone，指派开发者。

标签可分为两类，type 和 scope

- type: issue 的类型，如 `feature`, `bug`, `documentation`, `performance`, `support`
- scope: 修改文件的范围，如 `core`，`plugin: xx`，`deps: xx`， `example: xx`
...
- `bug`: 一旦发现可能是 bug 的问题，请打上 `bug`，然后等待确认，一旦确认是 bug，此 issue 会被再打上 `confirmed`。
  ...
  - bug 会在最低需要修复的版本进行修复，如是在 `0.2.0` 要修复的，而当前最新版本是 `0.3.0`，
  那么此 issue 还会被打上 `0.2`，`0.3`，代表需要修复到这些版本。
- `core`: 代表 issue 跟 core 内核相关。
- `plugin: xx`: 代表 issue 跟插件相关，如 `deps: react` 代表跟 `react` 插件相关。
- `deps: xx`: 代表 issue 跟 `dependencies` 模块相关，如 `deps: react` 代表跟 `react` 模块相关。
...
- how 部分必须包含详尽完整的操作步骤，必要时附上 **足够简单，可运行** 的范例代码，所有范例代码放在 [beidou/examples](https://github.com/alibaba/beidou/tree/master/examples) 库中。
...
如果你希望贡献代码，那么你可以创建分支修改代码提交 PR，beidou 开发团队会 review 代码合并到主干。
...
提交后就可以在 [beidou](https://github.com/alibaba/beidou/pulls) 创建 Pull Request 了。
...
你的代码风格必须通过 prettier 和 eslint，你可以运行 `$ yarn run lint` 本地测试。
...
- 如果功能点有新增或修改的，还需要关联文档 `doc` 和 `beidou-init` 的 PR
...
beidou 基于 [semver] 语义化版本号进行发布。

### 分支策略

`master` 分支为当前稳定发布的版本，`develop` 分支为下一个开发中的大版本。

- 只维护两个版本，除非有安全问题，否则修复只会 patch 到 `master` 和 `develop` 分支，其他更新推动上层框架升级到稳定大版本的最新版本。
- 所有 API 的废弃都需要在当前的稳定版本上 `deprecate` 提示，并保证在当前的稳定版本上一直兼容到新版本的发布。
- `master` 分支不设置 publish tag，上层框架基于 semver 依赖稳定版本。
- `develop` 分支设置 tag 为 `next`，上层框架可以通过 `beidou@next` 引用开发中的版本进行测试。
- beidou 持续维护的版本以 Milestone 为准，只要是开着的版本都会进行修复。
